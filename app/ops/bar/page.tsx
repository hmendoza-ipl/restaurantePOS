...
const statusStyle: Record<string, { label: string; bg: string; border: string; color: string; icon: string }> = {
  ...
  served:    { label: "Servido",    bg: "rgba(0,201,184,0.1)",    border: "rgba(0,201,184,0.25)",  color: "#00C9B8", icon: "🍹" },
};
...
export default function BarScreen() {
  ...
  const barItems = useMemo(() => {
    ...
    for (const o of orders.filter(x => x.status !== "closed")) {
      for (const it of o.items.filter((i: any) => i.station === "bar")) {
  ...
  const counts = useMemo(() => ({
    queued: barItems.filter(i => i.status === "queued").length,
    preparing: barItems.filter(i => i.status === "preparing").length,
    ready: barItems.filter(i => i.status === "ready").length,
  }), [barItems]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-[28px] p-5 flex items-start justify-between flex-wrap gap-4"
        ...
        <div>
          <div className="text-2xl font-bold flex items-center gap-2">🍹 Bar Screen</div>
          <div className="text-sm text-white/50 mt-1">Bebidas, cocteles y cafés · Haz clic para cambiar estado</div>
        </div>
      ...
      {barItems.length === 0 ? (
        <div className="py-20 text-center rounded-[22px]"
          ...
          <div className="text-xl font-semibold text-white/60">¡Bar al día!</div>
          <div className="text-sm text-white/30 mt-1">Sin bebidas pendientes</div>
        </div>
      ) : (
